        ...
        {steps.map((step, index) => (
          <motion.div key={index} variants={fadeInUp} className="text-center relative">
            ...
            {index < steps.length - 1 && (
              <div className="hidden lg:block absolute top-12 left-full w-16 h-1 bg-gradient-to-r from-current to-transparent transform translate-x-4 rounded-full opacity-30" />
            )}
          </motion.div>
        ))}
      </motion.div>
