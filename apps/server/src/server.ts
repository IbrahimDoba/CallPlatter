...
io.on("connection", (socket) => {
    ...
    try {
      ...
      if (!cfg) {
        ...
        try {
          ...
          if (user?.businessId) {
            logger.info(
              "Resolved userId to businessId for agent config lookup (WS)",
            ...
            try {
              ...
              if (biz) {
                logger.info(
                  "Resolved user via membership to business for agent config lookup (WS)",
              ...
              logger.warn(
                "Failed membership-based business resolution for agent config (WS)",
                { userId: user.id, e2 }
              );
            }
          }
          ...
          logger.warn(
            "Failed user->business fallback during agent config lookup (WS)",
      ...
      if (!cfg) {
        ...
        try {
          ...
          if (biz) {
            ...
            logger.info("Created default AIAgentConfig for business (WS)", {
              businessId: biz.id,
            });
          } else {
            logger.info(
              "No agent config and provided id is not a Business; using in-memory defaults (WS)",
              { providedId: businessId }
            );
          }
        } catch (e3) {
          logger.warn("Failed to auto-create default AIAgentConfig (WS)", {
      ...
      if (!cfg) {
        logger.info(
          "No agent config found for business, falling back to defaults",
          { businessId }
        );
      ...
      if (!firstMessageSent && currentAgentCfg?.firstMessage) {
        try {
          const buf = await aiService.textToSpeech(
            currentAgentCfg.firstMessage,
            currentAgentCfg
          );
          ...
          logger.info("Emitted firstMessage audio to client", {
            socketId: socket.id,
          });
        } catch (err) {
  ...
  socket.on("audio-chunk", async (audioData: ArrayBuffer) => {
    try {
      ...
      if (!firstMessageSent && currentAgentCfg?.firstMessage) {
        try {
          const greetBuf = await aiService.textToSpeech(
            currentAgentCfg.firstMessage,
            currentAgentCfg
          );
          socket.emit("audio-response", {
          ...
          firstMessageSent = true;
          logger.info(
            "Emitted firstMessage (safeguard) before processing first user audio",
            { socketId: socket.id }
          );
        } catch (err) {
          logger.error("Failed to synthesize firstMessage (safeguard)", {
            err,
          });
        }
      }
      ...
      const details = await aiService.processAudioConversationWithDetails(
        audioBuffer,
        currentAgentCfg
      );
      ...
      if (endDetected) {
        logger.info("END_CALL detected; notifying client to end call", {
          socketId: socket.id,
        });
  ...
  socket.on("new-call", (callData) => {
    ...
    socket.broadcast
      .to(`business-${callData.businessId}`)
      .emit("call-updated", callData);
  });
  ...
  socket.on("call-status-update", (updateData) => {
    ...
    // Broadcast to all clients in the business room
    socket.broadcast
      .to(`business-${updateData.businessId}`)
      .emit("call-updated", updateData);
  });
