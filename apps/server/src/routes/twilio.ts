  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (ngrokUrl) {
        ...
        return res.status(400).json({
          ok: false,
          error: "NGROK_URL environment variable is required for development. Please set up ngrok and add NGROK_URL to your server environment variables."
        });
      }
    }
    ...
    if (error instanceof Error && error.message.includes('not available')) {
      return res.status(400).json({ 
        ok: false,
        error: "This phone number is no longer available. Please select a different number.",
