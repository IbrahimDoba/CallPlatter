...
router.post("/config", async (req: SessionAuthenticatedRequest, res) => {
  try {
    ...
    if (body.businessName || body.businessDescription) {
      ...
      if (body.businessDescription) businessUpdate.description = body.businessDescription;
...
router.post("/elevenlabs/create", async (req: SessionAuthenticatedRequest, res) => {
  try {
    if (!req.user?.businessId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
...
router.put("/elevenlabs/update", async (req: SessionAuthenticatedRequest, res) => {
  try {
    if (!req.user?.businessId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
...
router.get("/elevenlabs/status", async (req: SessionAuthenticatedRequest, res) => {
  try {
    if (!req.user?.businessId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
...
router.post("/elevenlabs/sync", async (req: SessionAuthenticatedRequest, res) => {
  try {
    if (!req.user?.businessId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
